...
import { View, FlatList, Alert, ScrollView, Share, TouchableOpacity, StyleSheet, Image, Dimensions, KeyboardAvoidingView, Platform } from 'react-native'; 
...
type LocationData = { [province: string]: { [district: string]: string[] } }
...
interface Post {
  id: string; userName: string; content: string; mediaUrl?: string; mediaType?: 'image' | 'video'; likes: number; createdAt: any;
}

interface Group {
    id: string; name: string; desc: string; area?: string; province?: string; district?: string; ward?: string; members: number; image: string;
}
interface Comment {
    id: string; userName: string; text: string;
}

const PostMedia = ({ uri, type, onImagePress }: { uri: string, type: 'image' | 'video', onImagePress: (uri: string) => void }) => {
...
export default function CommunityScreen() {
  ...
  const uploadMedia = async (uri: string, path: string = 'community') => {
    try {
  ...
  const handleLike = async (postId: string) => { try { await updateDoc(doc(db, "community_posts", postId), { likes: increment(1) }); } catch {} };
  const handleSharePost = async (content: string) => { try { await Share.share({ message: `Mẹo sống xanh: ${content} - Environment App` }); } catch {} };
  ...
  const openCommentModal = (postId: string) => {
      ...
      onSnapshot(q, (snapshot) => { setComments(snapshot.docs.map(doc => ({id: doc.id, ...doc.data()} as Comment))); });
  };
  ...
  const handleCreateGroup = async () => {
      if (!newGroupName.trim() || !selectedProvince || !selectedDistrict) { Alert.alert("Thiếu thông tin", "Vui lòng nhập đủ thông tin."); return; }
      if (!user) return;
  ...
  const lightGreenButtonLabel = { color: 'white', fontWeight: 'bold' as const, fontSize: 16 };
    ...
    <View style={styles.container}>
      ...
      {tab === 'feed' ? (
        ...
        <View style={{flex: 1}}>
            ...
            <FAB icon="plus" style={{position: 'absolute', margin: 20, right: 0, bottom: 0, backgroundColor: '#0E4626'}} color="#fff" onPress={() => setVisibleCreateGroup(true)} label="Tạo nhóm" />
        </View>
      )}
